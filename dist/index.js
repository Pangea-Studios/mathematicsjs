...
class Equations {
    ...
    static evaluate(equation, variables) {
        ...
        const evaluateNoBrackets = (array) => {
            ...
            while (array.length !== 1) {
                ...
                for (let i = 0; i < array.length; i++) {
                    switch (array[i]) {
                        case '^':
                            (_a = array)[_b = i - 1] = Math.pow(_a[_b], array[i + 1]);
                            array.splice(i, 2);
                            break;
                        case '√':
                            if (Number.isInteger(Number(array[i - 1]))) {
                                (_c = array)[_d = i + 1] = Math.pow(_c[_d], 1 / array[i - 1]);
                                array.splice(i, 2);
                            }
                            else {
                                array[i] = Math.pow(array[i + 1], 0.5);
                                array.splice(i + 1, 1);
                            }
                            break;
                    }
        ...
        for (let i = 0; i < EquationArray1.length; i++) {
            if (Number.isInteger(Number(EquationArray1[i])) ||
                EquationArray1[i] === '.') {
        ...
        if (find(EquationArray2, '(') === find(EquationArray2, ')')) {
            while (find(EquationArray2, '(') > 0) {
                ...
                if (startPos !== -1 && endPos !== -1) {
                    const subEquation = EquationArray2.slice(startPos + 1, endPos);
    ...
    static solve(equation, variable) {
        if (equation === '' || !equation) {
            return 'Error';
        }
    }
    static rearrange(equation, makeSubject) {
        if (equation === '' || !equation) {
            return 'Error';
        }
    }
        ...
        if (result <= 0) {
            if (result >= -180) {
                if (result % 1 === 0) {
                    ...
                    return Summation.AdditiveSummation(`(((-1)^n)*(${Conversions.convertAngle(result, 'DEGREES', 'RADIANS')}^(2n+1)))/((2n+1)!)`, 0, accuracy, 1);
                }
            }
            else if (result <= -180) {
                if (result % 1 === 0) {
                    ...
                    return Summation.AdditiveSummation(`(((-1)^n)*(${Conversions.convertAngle(-result % 180, 'DEGREES', 'RADIANS')}^(2n+1)))/((2n+1)!)`, 0, accuracy, 1);
                }
            }
        }
        else {
...
export { Absolute, Averages, Circle, ComplexNumber, Constants, Conversions, Equations, Factorial, Fraction, Indices, Logarithms, Matrix, Summation, TrigonometryFunctions, add, angleEnum, angleUnits, areaEnum, areaUnits, convertBase, divide, energyEnum, energyUnits, gcd, gcd2, lengthEnum, lengthUnits, massEnum, massUnits, multiply, physicsFormulae, pressureEnum, pressureUnits, sineList, speedEnum, speedUnits, subtract, temperatureEnum, temperatureUnits, timeEnum, timeUnits, volumeEnum, volumeUnits };
