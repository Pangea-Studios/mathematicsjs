export class Logarithms {
	 ...
	 * @param {number} [accuracy=10] - the number of terms to use in the series approximation (default 10)
	 * @throws {Error} Invalid input. ln(x) is only defined for x > 0
	 * @return {number} the natural logarithm of the given number
	 */
	static ln(x: number, accuracy: number = 10) {
		if (x <= 0) {
	 ...
	 * @param {number} base - The base of the logarithm. Defaults to 10.
	 * @param {number} accuracy - The number of decimal places to calculate to. Defaults to 10.
	 * @return {number} - The calculated logarithm of x with base base, with the specified accuracy.
	 */
	static log(x: number, base: number = 10, accuracy: number = 10) {
